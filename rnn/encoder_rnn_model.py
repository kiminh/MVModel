...
from encoder_model_data import read_data
import numpy as np
import csv
...
tf.flags.DEFINE_string('acc_result_file', '/home1/shangmingyang/data/3dmodel/acc_result_modelnet40_lstm.csv', 'result save file')
...
def main(unused_argv):
    ...
    with tf.Session(config=config) as sess:
        ...
        while epoch <= training_epoches:
            batch = 1
            while batch * batch_size <= model_data.train.size():
                batch_x, batch_y = model_data.train.next_batch(batch_size, as_sequence=False)
                batch_x = batch_x.reshape((batch_size, n_steps, n_input))
            # Run optimization op (backprop)
                _, loss = sess.run([optimizer, cost], feed_dict={x: batch_x, y: batch_y, p: FLAGS.keep_prob})
                print("epoch " + str(epoch) + ", batch " + str(batch) + ", Minibatch Loss= " + \
                    "{:.6f}".format(loss))
                batch += 1

            if epoch % save_epoches == 0:
                #do test
                test_data = model_data.test.fcs.reshape((-1, n_steps, n_input))
                test_label = model_data.test.labels
                acc, predict = sess.run([accuracy, output_label], feed_dict={x: test_data, y: test_label, p: 1.0})
                #saver.save(sess, FLAGS.model_path)
                print("acc:", acc)
                acc_test_list.append([acc, acc_class(predict, test_label)])
            epoch += 1
        ...
        with open(FLAGS.acc_result_file, 'a') as f:
            writer = csv.writer(f)
            for r in acc_test_list:
                writer.writerow(r)
        #with open('result-1000epoch.txt', 'w') as f:
            #f.write('\n'.join([str(acc) for acc in acc_test_list]))

def acc_class(predict, target, mode="average_class"):
    class_target = []
    for onehot_class in target:
        class_target.append(np.argmax(onehot_class))
    predict, target = np.array(predict), np.array(class_target)
